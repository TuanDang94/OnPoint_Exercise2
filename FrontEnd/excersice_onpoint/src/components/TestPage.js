...
const TestPage = () => {
  ...
  const [isGetMovieFromDatabase, setIsGetMovieFromDatabase] = useState(false);
  ...
  const getAllMovie = async () => {
    try {
      setisShowLoading(false);
      setIsGetMovieFromDatabase(true);
      setMovie([]);
  ...
  const getMovieByUrl = () => {
    setisShowLoading(false);
    setIsGetMovieFromDatabase(false);
    console.log("Url: " + url);
    setMovie([]);
    getMovieData(url);
    setTotalSizeDataConfig(movies.length);
  };
